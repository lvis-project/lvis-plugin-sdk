...
- A `settings` onboarding action's `path` may now name a section as well as a
  tab: `"<tab>"` as before, or `"<tab>/<section>"`
  (`permissions/permissions-os-sandbox`). A tab is where a setting lives; the
  ...
  Mirrored from the host contract (`PluginOnboardingAction`) and the host
  schema, as every declaration in this package is.
