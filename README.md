...
- `MARKETPLACE_PUBLIC_KEYS` is a frozen record of `key_id → base64 raw 32-byte
  ed25519 public key`. It currently contains the committed transition keys
  `dev-v1` and `poc-v1`.
- Clients should accept a signature envelope if **at least one** signature
  verifies against any key in this map.
- `getTrustedMarketplacePublicKeys()` excludes test keys by default. Use
  `includeTestKeys: true` only for local/dev transition workflows.
- `MARKETPLACE_PRIMARY_KEY_ID` is the canonical current transition key id
  (`poc-v1`).
