...
describe("detectViaPrivateDnsProbe", () => {
  ...
  it("returns true when the host resolves", async () => {
    ...
    const result = await detectViaPrivateDnsProbe("portal.example.com", { timeoutMs: 500 });
    expect(result).toBe(true);
    expect(mockLookup).toHaveBeenCalledWith("portal.example.com");
  });

  it("returns false on ENOTFOUND", async () => {
    mockLookup.mockRejectedValueOnce(
      Object.assign(new Error("getaddrinfo ENOTFOUND portal.example.com"), {
        code: "ENOTFOUND",
      }),
    );
    const result = await detectViaPrivateDnsProbe("portal.example.com", { timeoutMs: 500 });
  ...
  it("returns false on timeout (fail-safe)", async () => {
    ...
    const result = await detectViaPrivateDnsProbe("portal.example.com", { timeoutMs: 50 });
    expect(result).toBe(false);
  });
  ...
  it("does NOT cache results — every call re-probes (off→on and on→off both work)", async () => {
    ...
    const r1 = await detectViaPrivateDnsProbe("portal.example.com", { timeoutMs: 500 });
    expect(r1).toBe(false);

    mockLookup.mockResolvedValueOnce({ address: "10.0.0.1", family: 4 });
    const r2 = await detectViaPrivateDnsProbe("portal.example.com", { timeoutMs: 500 });
    expect(r2).toBe(true);
  ...
  it("dedups concurrent callers on the same host within one probe window", async () => {
    ...
    const p1 = detectViaPrivateDnsProbe("portal.example.com", { timeoutMs: 5000 });
    const p2 = detectViaPrivateDnsProbe("portal.example.com", { timeoutMs: 5000 });
